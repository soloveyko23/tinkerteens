    ...
    (() => {
        ...
        function menuInit() {
            if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
                ...
                if (!e.target.closest(".sidebar") && !e.target.closest(".icon-menu")) functions_menuClose();
            }));
        }
        ...
        class Popup {
            ...
            eventsPopup() {
                document.addEventListener("click", function(e) {
                    ...
                    if (buttonOpen && !isNoClickInsidePopup && !isFlatpickrCalendar && !isModal && !fancyBox) {
                        e.preventDefault();
                    ...
                    if ((buttonClose || !e.target.closest(`.${this.options.classes.popupContent}`)) && this.isOpen && !isFlatpickrCalendar && !isModal && !fancyBox) {
                        e.preventDefault();
                        this.close();
                        return;
                    }
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) {
                new swiper_core_Swiper(".content-widgets-small .classes-calendar-schedule__slider", {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 3,
                    ...
                    speed: 800,
                    navigation: {
                        prevEl: ".classes-calendar-schedule .swiper-button-prev",
                        nextEl: ".classes-calendar-schedule .swiper-button-next"
                    },
                    breakpoints: {
                        320: {
                            slidesPerView: 4,
                ...
                new swiper_core_Swiper(".content-widgets-large .classes-calendar-schedule__slider", {
                    modules: [ Navigation ],
                    observer: true,
                    observeParents: true,
                    slidesPerView: 3,
                ...
                new swiper_core_Swiper(".content-widgets-large .classes-calendar__slider", {
                    observer: true,
                    observeParents: true,
                    slidesPerView: 3,
                    slidesPerGroup: 3,
                ...
                signInSlider.on("slideChange", (function() {
                    currentSlide = this.activeIndex;
                    isLastSlide = this.isEnd;
                    const mediaQuery = window.matchMedia("(max-width: 768px)");
                    if (mediaQuery.matches && currentSlide === 3) {
            ...
            document.addEventListener("DOMContentLoaded", (function() {
                ...
                if (inputReferralLink) {
                    ...
                    let clickHandler = function() {
                        ...
                        span.innerHTML = `${inputReferral.getAttribute("data-after-text")} <img width="16" height="16" src="/wp-content/themes/main/assets/img/icons-ios/check.png"/>`;
                        inputReferral.value = "";
        ...
        function moveDividerTabs() {
            window.onload = function() {
                ...
                tabSets.forEach((tabs => {
                    const tabsTitles = tabs.querySelectorAll("[data-tabs-titles] .tabs__title");
                    const divider = tabs.querySelector("[data-tabs-titles] .divider");
                    if (divider) {
                        const initialTab = tabs.querySelector(".tabs__title._tab-active");
        ...
        const closeNotificationButton = () => {
            document.addEventListener("DOMContentLoaded", (function() {
                const notifications = document.querySelectorAll(".critical-notification");
        ...
        const createDoughnutChart = canvasElement => {
            const percentage = canvasElement.getAttribute("data-percentage");
            if (!percentage) return;
            const colorsAttribute = canvasElement.getAttribute("data-colors");
            const percentageValue = parseFloat(percentage);
            const colorsArray = colorsAttribute ? colorsAttribute.split(",").map((color => color.trim())) : [ "#355CF7", "#E8EAF2" ];
        ...
        if (accountantChart) {
            ...
            const incomeData = [ 2e5, 15e4, 1e5, 24e4 ];
            ...
            document.getElementById("chart").addEventListener("mousemove", (event => {
                ...
                if (activePoints.length > 0) {
                    const datasetIndex = activePoints[0].datasetIndex;
                    customChart.data.datasets.forEach((dataset => {
                        dataset.borderColor = datasetIndex === 1 ? colors.blue.half : colors.red.half;
                    }));
                    customChart.data.datasets[datasetIndex].borderColor = customChart.data.datasets[datasetIndex].borderColor = datasetIndex === 1 ? colors.red.default : colors.blue.default;
                }
                customChart.update();
            }));
        }
        ...
        if (tabsBodyProducts) {
            const observer = new MutationObserver((mutationsList => {
                for (const mutation of mutationsList) if (mutation.type === "attributes" && mutation.attributeName === "hidden") if (tabsBodyProducts.hasAttribute("hidden")) shopStudentBonuses.classList.add("hide"); else shopStudentBonuses.classList.remove("hide");
            }));
            observer.observe(tabsBodyProducts, {
                attributes: true
            });
        }
        const block1 = document.getElementById("fullcalendarDaysScroll");
        const block2 = document.getElementById("fullcalendarInnerScroll");
        if (block1 && block2) {
            block1.addEventListener("scroll", (function() {
                block2.scrollLeft = block1.scrollLeft;
            }));
            block2.addEventListener("scroll", (function() {
                block1.scrollLeft = block2.scrollLeft;
            }));
            block1.addEventListener("wheel", (function(e) {
                e.preventDefault();
                block1.scrollLeft += e.deltaY;
            }), {
                passive: false
            });
        }
